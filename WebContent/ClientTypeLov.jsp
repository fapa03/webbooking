...
function getRow(){
	if(rowidflag=='true'){
		...
		window.opener.document.forms[0].tipoCliente.value = obj2.innerHTML;
		window.opener.document.forms[0].tipoClienteRef.value = obj1.innerHTML;
		window.opener.document.forms[0].nombre.focus();
		window.close();
	}else{
...
<div id="datadiv" style="BACKGROUND-COLOR: #cedee9; HEIGHT: 92pt; OVERFLOW: auto; WIDTH: 100%">
	<table align="left" width="100%" border=1 cellspacing="1" cellpadding="1">
		<tr>
			<td align="left"  width="20%" ><b><span style="width:100">Tipo Cliente</b></td>
            <td align="left" width="60%" ><b>Description</b></td>
       </tr>                
	<% 
		ArrayList result = (ArrayList) request.getAttribute("clientTypeRecords"); 
		if(result.size()==0){
	%>
	...
	%>	
		<tr id="tablerow<%=i%>" name=t1 style="CURSOR: hand" onclick="changeColor(this)">
				<td align="left" id="tablecol2<%=i%>" width="20%"><%=values.get("PM_PARM_CODE1")%></td>
				<td align="left" id="tablecol1<%=i%>" width="60%"><%=values.get("PM_VLUE1_DESC")%></td>                
       </tr>             
     <%
			}
		}
