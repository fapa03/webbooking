CREATE OR REPLACE PACKAGE BODY SIPPRB."PACK_WEB" IS
-------------------
/*General */
    ...
    END;
------------------
    FUNCTION fun_ftch_clnt_name(clnt_id VARCHAR2) RETURN VARCHAR2 IS
 ...
 FUNCTION FUN_FTCH_WT_VOL_AMNT(clnt_id VARCHAR2, orgn_brnc VARCHAR2,dest_brnc VARCHAR2,trif_slab VARCHAR2,
    ...
    BEGIN
        ...
        ELSE
            ...
            IF lc_weight*lc_ftch_kg > LC_VOLUME * lc_ftch_cum THEN
               RETURN (lc_weight*lc_ftch_kg);
            ELSE
...
------------------------------------------------------------------------
    ...
    BEGIN
        ...
        END LOOP;
        --DELETE FROM WEB_MNFT_DETL WHERE MD_MNFT_NO = manifestNumber;
        UPDATE WEB_MNFT_DETL SET MD_DELETE_FLAG ='Y', MDFD_BY = userid, MDFD_ON = ld_date WHERE MD_MNFT_NO = manifestNumber;
        /*YA NO SE UTILIZA LA TABLA WEB_CTRL_EMAIL A NIVEL DE MANIFIESTO, AHORA ES POR GUIA*/
        --DELETE FROM WEB_CTRL_EMAIL WHERE CE_MNFT_NO = manifestNumber;--AAP02
...
CURSOR CUR_REQM_TEXT(LV_INFO VARCHAR2) IS
  SELECT CR_SRVC_ID,CR_REFR_SRVC_ID,CR_TYPE,CR_CONF_FLAG,CR_DEFA_TEXT,
         CR_INFO_OR_REQM,CR_PRNT_IN_GUIA,CR_PRNT_IN_ACK,CR_BOOK_OR_DLVY_OR_BOTH
            ...
            INSERT INTO BOK_GUIA_SRVC_ITEM_REQM (GI_SRVC_ID,GI_REFR_SRVC_ID,GI_GUIA_NO,
                                      ...
                                      VALUES
                                             (I.CR_SRVC_ID,
                                                     ...
                                                     GUIA_NO,
                                                                                         '1',
                                                                                         I.CR_DEFA_TEXT||';',--AAP02 SE CONCATENÓ ';'
                                                                                            '1',
                                                     'R',
                                                     SYSDATE,
                                                                                           ORGN_CLNT_ID,
...
------------------
/* For WEB Booking */
/* This Procedure is used to Check whether service 'RAD' is applicable to the
   Destination City, Colony and Site Selected  - Created by ABARJONA on 11/09/2014 */
...
END;
------------------
FUNCTION fun_chk_dstn_brnch (orgn_brnc VARCHAR2, dest_brnc VARCHAR2) RETURN NUMBER IS
    CURSOR cur_actv_trif (mdul_id VARCHAR2, parm_type VARCHAR2, code1 VARCHAR2, vlue1 NUMBER)IS
        SELECT COUNT(*)
            FROM SYS_PARM_MSTR
            WHERE pm_mdul_id = mdul_id
...
FUNCTION fun_ftch_trif_exist(clnt_id VARCHAR2, orgn_brnc VARCHAR2, dest_brnc VARCHAR2,
    ...
    CURSOR cur_ftch_trif_exist_new (ol_id varchar, lv_lc_BD_DSTN number) IS    
    SELECT TAR_BOX_TYPE FROM WEB_CLNT_TARIFF 
        WHERE TAR_CLNT_ID = clnt_id 
        AND TAR_OL_ID = ol_id 
        AND TAR_FROM_KMS >= lv_lc_BD_DSTN AND TAR_TO_KMS <= lv_lc_BD_DSTN;
    
    lc_TRIF_SLAB WEB_CLNT_SRVC_TRIF.WT_TRIF_SLAB%TYPE;
    lc_BD_DSTN SYS_BRNC_DSTN.BD_DSTN%TYPE;
    lc_TAR_BOX_TYPE WEB_CLNT_TARIFF.TAR_BOX_TYPE%TYPE;
BEGIN
   IF trif_type = 'W' THEN
    IF substr(orgn_brnc,1,3) = substr(dest_brnc,1,3) THEN
        lc_BD_DSTN := 0;
    ELSE
        OPEN cur_trif_dist;
        FETCH cur_trif_dist INTO lc_BD_DSTN;
        IF (cur_trif_dist%NOTFOUND) THEN
            lc_BD_DSTN := 0;
        END IF;
        CLOSE cur_trif_dist;
    END IF;
    OPEN cur_ftch_trif_exist_new('PQE',lc_BD_DSTN);
    FETCH cur_ftch_trif_exist_new INTO lc_TAR_BOX_TYPE;
    IF (cur_ftch_trif_exist_new%NOTFOUND) THEN
        lc_BD_DSTN := 0;
        RETURN 'N';
    ELSE
        RETURN 'Y';
    END IF;
    CLOSE cur_ftch_trif_exist_new;
   ELSE
...
PROCEDURE pro_ftch_addr_postal(am_defn_type VARCHAR2, am_ref_no VARCHAR2, am_gcode VARCHAR2,
   ...
   c11 OUT VARCHAR2,c12 OUT VARCHAR2,c13 OUT VARCHAR2,c14 OUT VARCHAR2,c15 OUT VARCHAR2,c16 OUT VARCHAR2) IS
    u1 VARCHAR2(50);u2 VARCHAR2(50);
    u3 VARCHAR2(50);u4 VARCHAR2(50);u5 VARCHAR2(50);
    u6 VARCHAR2(50);u7 VARCHAR2(50);c1 VARCHAR2(50);
    c2 VARCHAR2(50);c3 VARCHAR2(50);c4 VARCHAR2(50);
...
FUNCTION FUN_GET_TARIFF (WEIGHT number,VOLUME number) RETURN varchar2 IS
    lc_Tarif_kg VARCHAR2(50):='T7';
    lc_Tarif_Cum VARCHAR2(50):='T7';
    lc_Tarif VARCHAR2(50):='T7';
    cursor cur_Tarif_kg is 
    SELECT TI_FROM_VLUE,TI_TO_VLUE,TI_TRIF_ID
          FROM ACT_TRIF_ID
          WHERE TI_SRVC_ID='SHP-G'
          AND TI_FACT='KG';      

    cursor cur_Tarif_Cum is 
    SELECT TI_FROM_VLUE,TI_TO_VLUE,TI_TRIF_ID
        FROM ACT_TRIF_ID
        WHERE TI_SRVC_ID='SHP-G'
        AND TI_FACT='CUM';                 
...
BEGIN
    FOR rec IN cur_Tarif_Kg
        LOOP
         IF WEIGHT >=REC.TI_FROM_VLUE AND WEIGHT <=REC.TI_TO_VLUE THEN
            lc_Tarif_Kg:=REC.TI_TRIF_ID;
         END IF;
        END LOOP;
    FOR rec IN cur_Tarif_Cum
       LOOP
        IF VOLUME >=REC.TI_FROM_VLUE AND VOLUME <=REC.TI_TO_VLUE THEN
            lc_Tarif_Cum:=REC.TI_TRIF_ID;
        END IF; 
       END LOOP;   
    If substr(lc_Tarif_Cum, 1) >= substr(lc_Tarif_kg, 1)  Then
      lc_Tarif := lc_Tarif_Cum;
    Else
      lc_Tarif := lc_Tarif_Kg;
    End If;
    Return lc_Tarif;
END;

PROCEDURE INSER_ORDER_STUS (lc_ct_trns_type VARCHAR2, l_ct_trns_no VARCHAR2, LV_ORDER_NO VARCHAR2, LV_CLINT_ORGN VARCHAR2, LV_MOTIV VARCHAR2, LV_ORDER_TYPE VARCHAR2, LV_CURR_LOCA VARCHAR2,user_id VARCHAR2 ) IS
    l_serl_no RAD_ORDER_STUS.Rs_serl_no%TYPE;

    l_guia_ulod NUMBER := 0; 
    l_guia_ulod_wght NUMBER := 0;
    l_guia_ulod_vlum NUMBER := 0;
    l_guia_ulod_decl NUMBER := 0;
    l_date           date   := SYSDATE; --TRUNC(SYSDATE); value replaced on 13/11/2006
    

        
    cursor get_serl_no(ARG_ORDER_NO VARCHAR2) is
        SELECT NVL(MAX(RS_SERL_NO),0) + 1 
        FROM RAD_ORDER_STUS 
        WHERE RS_ORDER_NO = ARG_ORDER_NO;
        
  
BEGIN        

        open get_serl_no(LV_ORDER_NO);
        fetch get_serl_no into l_serl_no;
        close get_serl_no;--till here

 -- MESSAGE (LV_ORDER_NO);
        BEGIN
    
            INSERT INTO RAD_ORDER_STUS
            (    RS_ORDER_NO,
                RS_STUS_CODE,
                RS_SERL_NO,
                RS_COM_TRNS_REF_CODE,
                RS_CTT_TRNS_TYPE,
                RS_BRNC_LOCA,
                RS_CLNT_ORGN,            
                crtd_on,
            ...
            (    LV_ORDER_NO,
              lc_ct_trns_type,
                l_serl_no,
                l_ct_trns_no,
                lc_ct_trns_type, --parameter cursor on 07/03/2002
              LV_CURR_LOCA, --:GLOBAL.BRANCH_ID, --LV_curr_loca,----Code Modified For DC on 10-Apr-07 :global.site_id, modified on 15/03/2008
                LV_CLINT_ORGN,
