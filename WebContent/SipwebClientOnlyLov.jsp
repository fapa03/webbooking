...
function getRow1(){
	...
	if(rowidflag=='true'){
		...
		if(window.opener.document.forms[0].destinationclave!=null) {			
			...
			window.opener.document.forms[0].clientName.value = obj2.innerHTML;			
			window.opener.document.forms[0].clientId.value = obj1.innerHTML;		
		} else {		
			window.opener.document.forms[0].destclave.value=obj1.innerHTML;
			window.opener.document.forms[0].destnombre.value=obj2.innerHTML;
		}
		window.opener.document.forms[0].allowNewInvoice.value="S";
		<% if (request.getAttribute("type") != null && !request.getAttribute("type").toString().equalsIgnoreCase("sipwebclientonlypay")){ %>
			window.opener.document.forms[0].productIdSat.value="";
			window.opener.document.forms[0].productDescSat.value="";
		<%}%>
		window.opener.document.forms[0].banCerrar.value = "false";
		window.opener.document.forms[0].currentTask.value="checkclientcredit";
...
<form name=frm>
<div id="datadiv" style="HEIGHT: 149pt; OVERFLOW: auto; ">
	<table align="left" width="520" border=1 cellspacing="0" cellpadding="0" class="tablaDetallev2" >
		<tr width="520">
	...
	%>
			...
			<tr>
				<td align="center" colspan=2 width="550"><font size="5" color="red" >No Records</font></td>
			</tr>
	<%
		}else{
			HashMap values = null;
