 ...
 * Fecha de Creacion: 15/08/2013
 * Compañia: KUMARAN.
 * Descripcion del programa: js para validaciones y procesos de CONVERSION GUIAS PREPAGO.
...
function isNumber(evt,txt) {
 ...
 function validationTypeServicesSEG(){
	 if (document.forms[0].shipType.value == 'ST') {
		 if(document.forms[0].weightVolumetric.value == '0' || document.forms[0].weightVolumetric.value == '') {
			document.forms[0].volL.value = '0';
			document.forms[0].volH.value = '0';
			document.forms[0].volW.value = '0';
		 }
	} else {
		document.forms[0].weight.readOnly = false;
	}
 }
...
function setAcuseText() {
	document.forms[0].actType.value = document.forms[0].acusederecibo.value;
	document.forms[0].actTypeCurrent.value = document.forms[0].acusederecibo.value;
}
function extChk() {
	if (document.forms[0].extCheck.checked) {
		var parametro = document.forms[0].tarifa.value;
		var identificador = parametro.substring(0, 2);
		if (identificador != 'T7') {
			document.forms[0].ext.value = "S";
		} else {
			alert("No se puede escoger entrega a Zona Extendida en el tipo de Tarifa T7");
		}
	} else {
...
function eadChk() {
	if (document.forms[0].eadCheck.checked == true) {
		if (document.forms[0].ext.value == "S") {
			// alert("No se puede incluir ese servicio si se escogio
			// entrega en Zona Extendida");
			document.forms[0].ead.value = "N";
			document.forms[0].eadCheck.checked = false;
		} else
			document.forms[0].ead.value = "S";
	} else {
		document.forms[0].ead.value = "N";
	}
}
...
function pesoT7Check() {
	return true;
}

function radChk() {
	if (document.forms[0].radCheck.checked == true) {
		document.forms[0].rad.value = "S";
	} else {
		document.forms[0].rad.value = "N";
	}
}
function valorCheck() {
	if (document.forms[0].insCheck.checked == true) {
		document.forms[0].insAmt.disabled = false;
	} else {
		document.forms[0].insAmt.disabled = true;
		document.forms[0].insAmt.value = document.forms[0].insOrigVal.value;
	}
}

function weightEventCheck()
{
	if(document.forms[0].weightCheck.checked==true)
	{
		document.forms[0].weightUpdate.disabled=false;
	}
	else
	{
		document.forms[0].weightUpdate.disabled=true;
	}
}

function acuseCheck() {
	if (document.forms[0].actCheck.checked == true) {
		document.forms[0].acusederecibo.disabled = false;
	} else {
		document.forms[0].acusederecibo.disabled = true;
		document.forms[0].actType.value =
		(document.forms[0].ackOrigVal.value === null ||
		document.forms[0].ackOrigVal.value === '' ||
		document.forms[0].ackOrigVal.value === 'ACK-N')
    ? 'ACK-N' : document.forms[0].ackOrigVal.value;
		document.forms[0].actTypeCurrent.value = document.forms[0].actType.value;
	}
}
...
function facturarCambios() {	
	/*if (document.forms[0].cash != null) {
		if ((document.forms[0].cash.checked)
				|| (document.forms[0].credit.checked)) {
			if (document.forms[0].cash.checked)
				document.forms[0].payMode.value = "ToPay";
			if (document.forms[0].credit.checked)
				document.forms[0].payMode.value = "PAID";
	...
	if (document.forms[0].fiscaladdresscode.value == "") {
		alert("POR FAVOR CONFIRME LA DIRECCION FISCAL DEL CLIENTE Y VUELVA A INTENTAR");
		openLov('sipwebfiscaladdress');
		return false;
	}
	
	var tipoTarifa = document.forms[0].tarifa.value;
	var identificador = tipoTarifa.substring(0, 2);
	if (identificador == "T7" & document.forms[0].extCheck.checked) {
		alert("No se puede seleccionar Tarifa T7 y Zona extendida en el mismo documento");
		return false;
	}
	...
	if (document.forms[0].aceptarNuevosCargos.value == "S") {
		...
		var vEnvio = new Number(document.forms[0].nuevoValorEnvio.value);
		cadenaCambios = cadenaCambios + "\r\n Cargo por envio:    $" + vEnvio.toFixed(2);
		var vEAD = new Number(document.forms[0].nuevoValorEAD.value);
		cadenaCambios = cadenaCambios + "\r\n Cargo por EAD:      $" + vEAD.toFixed(2);
		var vRAD = new Number(document.forms[0].nuevoValorRAD.value);
		cadenaCambios = cadenaCambios + "\r\n Cargo por RAD:      $" + vRAD.toFixed(2);
		cadenaCambios = cadenaCambios + "\r\n Cargo por Zona Ext: $" + document.forms[0].nuevoValorEXT.value;
		var vINS = new Number(document.forms[0].nuevoValorSeguro.value);
		cadenaCambios = cadenaCambios + "\r\n Cargo por Seguro:   $" + vINS.toFixed(2);
		var vAcuse = new Number(document.forms[0].nuevoValorAcuse.value);
		cadenaCambios = cadenaCambios + "\r\n Cargo por Acuse:    $" + vAcuse.toFixed(2);
		var total = parseFloat(document.forms[0].nuevoValorEnvio.value)
				+ parseFloat(document.forms[0].nuevoValorEAD.value)
				+ parseFloat(document.forms[0].nuevoValorRAD.value)
				+ parseFloat(document.forms[0].nuevoValorEXT.value)
				+ parseFloat(document.forms[0].nuevoValorSeguro.value)
				+ parseFloat(document.forms[0].nuevoValorAcuse.value);
		var vtotal = new Number(total);
		cadenaCambios = cadenaCambios + "\r\n Subotal:            $ " + vtotal.toFixed(2);
		var vValorIVA = parseFloat(document.forms[0].valorIVA.value);
		//var vValorRET = parseFloat(document.forms[0].valorRET.value);
		var vValorRetAmount = parseFloat(document.forms[0].valorRetAmount.value);
		
		var iva = parseFloat(total) * (vValorIVA / 100);
		var ret = 0;
		
		if ((document.forms[0].clientType.value == "C" || document.forms[0].clientType.value == "G") && (document.forms[0].retieneCmpy.value == "Y" || document.forms[0].ivaRetTemplate.value == "Y")) {
			//ret = parseFloat(vEnvio) * (vValorRET / 100);
			ret = vValorRetAmount;
		} else {
			ret = 0;
		}
		...
		var ttotal = parseFloat(total) + parseFloat(iva) - parseFloat(ret);
		var fttotal = new Number(ttotal);
		cadenaCambios = cadenaCambios + "\r\n ==================================";
		cadenaCambios = cadenaCambios + "\r\n Total Aprox.:       $ " + fttotal.toFixed(2);
		if (document.forms[0].actType.value == "ACK-N") {
			cadenaCambios = cadenaCambios + "\r\n No incluye el cargo por Acuse de Recibo";
		}
		if (document.forms[0].tarifaT7.value != "") {
			cadenaCambios = cadenaCambios + "\r\n Se aplico la tarifa especial:  " + document.forms[0].tarifaT7.value + "\r\n";
		...
		if (i == true) {
			if (fttotal<=0) {
				...
				if(document.forms[0].weightAfterModification.value != '') {
					document.forms[0].weight.value = document.forms[0].weightAfterModification.value;
		        	document.forms[0].volume.value = document.forms[0].volumeAfterModification.value;
				}
				document.forms[0].modificar.disabled = true;
				document.forms[0].confirmar.disabled = false;
		...
		alert("No se han hecho cambios al rastreo a documentar. No se puede generar factura");
	}
}

function disableElems(state){
	document.forms[0].zone.disabled=state;
	document.forms[0].tarifa.disabled=state;
	document.forms[0].weight.disabled=state;
	document.forms[0].volume.disabled=state;
	document.forms[0].volL.disabled=state;
	document.forms[0].volH.disabled=state;
	document.forms[0].volW.disabled=state;
	document.forms[0].weightVolumetric.disabled=state;
	document.forms[0].packType.disabled=state;
	document.forms[0].content.disabled=state;
	document.forms[0].productDescSat.disabled=state;
	document.forms[0].shippingType.disabled=state;
	document.forms[0].lovbutton3.disabled=state;
}

function changeGuia() {
	if (document.forms[0].trackingNoEnd.value != "") {
		alert("No se puede modificar los datos de un rango de rastreos. Ingrese solo UN rastreo a modificar");
	} else if (document.forms[0].oldRefNo.value != "") {
		...
		if (document.forms[0].modificar.value == "Modificar Documentacion") {
			if (document.forms[0].shipType.value != 'ST') {
				document.forms[0].weightUpdate.readonly = false;
				document.forms[0].weightCheck.disabled = false;
		        document.forms[0].weight.value = document.forms[0].weightOriginal.value;
		        document.forms[0].weight.readOnly=true;
			} else {
				document.forms[0].weightUpdate.readonly = true;
				document.forms[0].weightCheck.disabled = true;
		        document.forms[0].weight.readOnly=false;
			}
			if (document.forms[0].insAmt.value == "0" || document.forms[0].insAmt.value == '') {
				document.forms[0].insAmt.readonly = false;
				document.forms[0].insCheck.disabled = false;
			} else {
				document.forms[0].insAmt.readonly = true;
				document.forms[0].insCheck.disabled = true;
			}
			if (document.forms[0].actType.value != "") {
				document.forms[0].actCheck.disabled = true;
			} else {
				document.forms[0].actCheck.disabled = false;
			}
			document.forms[0].zone.readonly = false;
			document.forms[0].volume.readonly = false;
			...
			document.forms[0].facturar.disabled = true;
			if (document.forms[0].rad.value == "N") {
				document.forms[0].radCheck.disabled = false;
			}
			if (document.forms[0].ead.value == "N") {
				document.forms[0].eadCheck.disabled = false;
			}
			if (document.forms[0].ext.value == "N") {
				document.forms[0].extCheck.disabled = false;
			}
			document.forms[0].zoneCombo.disabled = false;
			if(document.forms[0].shipType.value == 'ST') {
				document.forms[0].tarifaCombo.disabled = false;
		    }
			...
			if (document.forms[0].volume.value == "") {
				alert("No se capturo valor del Volumen");
				return false;
			}
			if (document.forms[0].weight.value == "") {
				alert("No se capturo valor del Peso");
				return false;
			}			
			...
			if (document.forms[0].shipType.value != 'ST') {
				if(document.forms[0].ext.value=="S") {
					alert("No se permite seleccionar zona extendida con el tipo de envío seleccionado");
					return;
	        	}

				if(document.forms[0].weightCheck.checked) {
					var vWeightIncrement=new Number(document.forms[0].weightUpdate.value);
		     		var vWeightOriginal=new Number(document.forms[0].weightOriginal.value);
		     		var vWeighMax=new Number(document.forms[0].pesoMAX.value);
		     		var vWeightVolumetrico = new Number(document.forms[0].weightVolumetricMAX.value);
		     		if(vWeighMax < vWeightVolumetrico) {
		     			vWeighMax = vWeightVolumetrico;
		     		}
					if(((vWeightOriginal+vWeightIncrement)) > vWeighMax) {
						alert("El peso real cubierto por la guia ("+vWeightOriginal+" kg.) mas el incremento del peso CAPTURADO ("+vWeightIncrement+" kg.) supera el peso maximo permitido por el tipo de envío.\nEl peso maximo  real para este tipo de envio es de "+document.forms[0].pesoMAX.value+" kg.\n\nTopes de medidas: Largo = "+document.forms[0].volLMAX.value+", Ancho ="+document.forms[0].volWMAX.value+", Alto ="+document.forms[0].volHMAX.value+"\nTope peso volumetrico "+document.forms[0].weightVolumetricMAX.value +" kg.");
		     			return false;
		     		}
				}
			}
			document.forms[0].weightAfterModification.value = document.forms[0].weight.value;
	        document.forms[0].volumeAfterModification.value = document.forms[0].volume.value;
			eadChk();
			radChk();
			extChk();
			
			if (parseFloat(document.forms[0].insAmt.value) > parseFloat(document.forms[0].maxDeclAmnt.value)){
			...
			if (document.forms[0].eadOrigVal.value == document.forms[0].ead.value &&
	        		 document.forms[0].radOrigVal.value == document.forms[0].rad.value &&
	        		 document.forms[0].extOrigVal.value == document.forms[0].ext.value && 
	        		 document.forms[0].zoneOrigVal.value == document.forms[0].zone.value &&
	        		 document.forms[0].tarifOrigVal.value == document.forms[0].tarifa.value &&
	        		 document.forms[0].volumeOriginal.value == document.forms[0].volume.value.replace(',','.') &&
	        		 document.forms[0].weightOriginal.value == document.forms[0].weight.value ){
				var ncambio = 0;
	     		if (document.forms[0].insAmt.value == '0' && document.forms[0].insOrigVal.value == ''
	     			|| document.forms[0].insAmt.value == document.forms[0].insOrigVal.value){
	     			ncambio++;
	     		}
	     		if (document.forms[0].ackOrigVal.value == document.forms[0].actType.value ||
	     				document.forms[0].actType.value == "ACK-N" && document.forms[0].ackOrigVal.value == ""){
	     			ncambio++;
	     		}
	     		
	     		if (document.forms[0].shippingType.value != 'STANDARD'){
		        	 if (document.forms[0].weightUpdate.value == document.forms[0].weightOriginal.value &&
		        			parseFloat(document.forms[0].weightUpdate.value) > 0 ){
		        		 ncambio++;
		        	 }
	     		}
	     		if (ncambio == 2 && document.forms[0].shippingType.value == 'STANDARD'){
	     			alert("No se ha modificado el rastreo.");
					return false;
	     		}else if(ncambio==3 && document.forms[0].shippingType.value != 'STANDARD'){
	     			alert("No se ha modificado el rastreo.");
					return false;
	     		}
	         } else if (document.forms[0].shippingType.value != 'STANDARD'){
	        	 if (document.forms[0].weightUpdate.value == document.forms[0].weightOriginal.value &&
	        			parseFloat(document.forms[0].weightUpdate.value) > 0 ){
	        		 alert("No se ha modificado el rastreo.");
					 return false;
			...
			document.forms[0].insCheck.disabled = false;
			document.forms[0].actCheck.disabled = false;
			if (document.forms[0].actCheck.checked) {
				document.forms[0].acusederecibo.disabled = false;
			} else {
				document.forms[0].acusederecibo.value = document.forms[0].ackOrigVal.value;
				document.forms[0].acusederecibo.disabled = false;
			}
			document.forms[0].zone.readonly = true;
			document.forms[0].weight.readonly = true;
			document.forms[0].volume.readonly = true;
			document.forms[0].modificar.value = "Modificar Documentacion";
			document.forms[0].confirmar.disabled = true;
			document.forms[0].limpiar.disabled = false;
			document.forms[0].facturar.disabled = false;
			document.forms[0].radCheck.disabled = true;
			document.forms[0].eadCheck.disabled = true;
			document.forms[0].extCheck.disabled = true;
			document.forms[0].zoneCombo.disabled = true;
			document.forms[0].tarifaCombo.disabled = true;
			document.forms[0].modFormnoFlag.value = "N";
			document.forms[0].aceptarNuevosCargos.value = "S";
			document.forms[0].currentTask.value = "guidepreviewcost";
			document.forms[0].banCerrar.value = "false";//bandera para validar cuando se cierre la ventana desde "x" de navegador
			disableElems(false);
			document.forms[0].submit();
		}
...
function onChangeTarifa(valor) {
	if (Number(valor.charAt(1)) < Number(document.forms[0].tarifa.value.charAt(1)))
		alert("No se puede seleccionar una tarifa menor a la actual");
	else {
		var parametro = valor.toLowerCase();
		var identificador = valor.substring(0, 2);
		var parametrokilos = parametro + 'kg';
		if (identificador != 'T7') {
			document.forms[0].volume.value = document.forms[0].elements[parametro].value;
			document.forms[0].weight.value = document.forms[0].elements[parametrokilos].value;
			document.forms[0].tarifa.value = valor;
			document.forms[0].volume.readOnly = true;
			document.forms[0].weight.readOnly = true;
		} else {
			if (document.forms[0].extCheck.checked) {
				alert("No se puede seleccionar la Tarifa T7 para entrega en Zona Extendida");
				return false;
			} else {
				document.forms[0].volume.value = "";
				document.forms[0].weight.value = "";
				document.forms[0].tarifa.value = valor;
				document.forms[0].volume.disabled=false;
				document.forms[0].volume.readOnly = false;
...
function onChangeZona(valor) {
	if (Number(valor) < Number(document.forms[0].zone.value))
		alert("No se puede seleccionar un valor menor al de la zona actual");
	else
		document.forms[0].zone.value = valor;
	return;
}
...
function verifyDelvryType() {
	if ((document.forms[0].deliveryType.value != "O")) {
		...
		if ((document.forms[0].deliveryType.value == "H")
				&& ((document.forms[0].ead.value == "N") && (document.forms[0].ext.value == "N"))) {
			alert('EL RASTREO NO INCLUYE EL SERVICIO DE EAD');
			return;
		}
	} else {
	...
	if (trim(document.forms[0].destSite.value) == "") {
		...
		return false;
	} else if (parseFloat(document.forms[0].weight.value) > parseFloat(document.forms[0].pesoMaximoT7.value)) {
		alert("El peso indicado supera al valor máximo permitido");
		return false;
	}
...
function openLov(str) {
	...
	if (str == 'clientwithsite') {
		if (destsite == '') {
			alert('SELECCIONE LA CIUDAD DESTINO');
			return;
		} else if ((document.forms[0].destclave.value == "") && (destnombre == "")) {
			alert("Capture el nombre del cliente");
	...
	if (str == 'sipwebfiscaladdress') {
		...
		window.open('lov.do?type=' + str + '&clientname='+ document.forms[0].clientName.value, 'abc',
...
function convert() {
	...
	if (document.forms[0].orgiencolonia1.value == "") {
		...
		if (confirm("LOS DATOS ESTE RASTREO SE MODIFICARON Y NO SE PUEDE REALIZAR LA CONVERSION. \u00BFDESEA RECARGAR VALORES ORIGINALES?")){
			document.forms[0].aceptarNuevosCargos.value = "N";
			document.forms[0].payMode.value = "PAID";
			document.forms[0].allowNewInvoice.value = "N";
			loaddetail();
			return false;
		}else{
			return false;
		...
		return false;
	} else if (document.forms[0].rad.value != "S") {
		if (!confirm("EL RASTREO NO CUENTA CON SERVICIO DE RECOLECCION A DOMICILIO. ¿DESEA CONTINUAR?")) {
			return false;
		}
	}
	...
	if ((document.forms[0].deliveryType.value != "O")) {
		...
		/*
		 * SE AGERGA FUNCION PARA QUE REALICE VALIDACION DE NO ENTREGAR A OCURRE
		 * EN ZONA EXTENDIDA
		 */
	...
	if (document.forms[0].volume.value == "X") {
		alert('CAPTURE EL VALOR DEL VOLUMEN DE EL RASTREO');
		return false;
	}
	if (document.forms[0].weight.value == "X") {
		alert('CAPTURE EL VALOR DEL VOLUMEN DE EL RASTREO');
		return false;
	}
	...
	if (document.forms[0].reqAcuse.value == 'N'
		...
		alert('CLIENTE ORIGEN NO TIENE REQUERIMIENTOS DE ACUSE CAPTURADOS. FAVOR DE CAPTURAR REQUERIMIENTOS EN CATALOGO DE CLIENTES.');// AAP02
		return false;
	}// AAP02
	if (document.forms[0].reqAcuseXT.value == 'N'
		&& document.forms[0].actType.value == 'ACK-X') {// AAP02
		alert('CLIENTE ORIGEN NO TIENE REQUERIMIENTOS DE ACUSE XT CAPTURADOS. FAVOR DE CAPTURAR REQUERIMIENTOS EN CATALOGO DE CLIENTES.');// AAP02
		return false;
	}

	/*
	 * validacion para monto maximo asegurado (en caso de que se haya
	 * capturado.)
	 */
	var montoMaxOl = document.forms[0].montoMaxOl.value;// AAP02
	if (montoMaxOl != "") {// /AAP02
		alert('MONTO MAXIMO A CAPTURAR DE VALOR DECLARADO PARA ESTE ENVIO: $'+ montoMaxOl);// AAP02
		return false;// AAP02
	}// AAP02
	...
	if(document.forms[0].shipType.value == 'ST') {
		value = new Number(document.forms[0].weight.value.replace(",", "."));
		valueMAX = new Number(document.forms[0].weightOriginal.value.replace(",", "."));
		if (value > valueMAX) {
			if(document.forms[0].shipType.value == 'ST') {
				alert("EL PESO MAXIMO PERMITIDO QUE CUBRE LA GUIA ES DE " + valueMAX +" kg.");
			}
			document.forms[0].weight.focus();
			return false;
		}
		value = new Number(document.forms[0].volume.value.replace(",", "."));
		valueMAX = new Number(document.forms[0].volumeOriginal.value.replace(",", ".") );
		if (value > valueMAX) {
			if(document.forms[0].shipType.value == 'ST') {
				alert("EL VOLUMEN MAXIMO PERMITIDO QUE CUBRE LA GUIA ES DE " + valueMAX +" kg.");
			}
			document.forms[0].volume.focus();
			return false;
		}
	}
	/*validacion para referencia obligatoria*/
...
function resetfields() {
	...
	document.forms[0].modFormnoFlag.value = "N";
	document.forms[0].insAmt.value = 0;
	...
	if (document.forms[0].checkRefDir.value == 'Y'
			|| document.forms[0].checkRefDir.value == 'V') {// AAP02
		document.getElementById('divRefDom').style.visibility = 'hidden';// AAP02
		document.getElementById('divRefDom').style.position = 'absolute';// AAP02
		document.forms[0].destRefDom.value = '';// AAP02
		document.forms[0].checkRefDir.value = '';// AAP02
	}// AAP02
...
/*
 * function focus() { if(<%=session.getAttribute("zoneFocus")%> == '1') {
 * document.forms[0].destclave.focus(); } }
 */
...
function validCamposZE() {
	...
	if (document.forms[0].isExtendedZone.value == "S") {
		...
		if (document.forms[0].checkTelDir.value == 'Y') {
			document.forms[0].destinationtelefono.readOnly = false;// AAP02
		} else if (document.forms[0].checkTelDir.value == 'V') {
			document.forms[0].destinationtelefono.readOnly = true;// AAP02
		}
	}
}
...
function emailValidate(checkObj, txtObj, msj) {// AAP04
	...
	if (eval("document.forms[0]." + checkObj).checked) {
		if (eval("document.forms[0]." + txtObj).value == "") {// AAP04
			...
			// eval("document.forms[0]."+txtObj).value = "";//AAP04
			var at = ";";
...
function submitFormMain(accion, mensaje, params) {
	...
	document.forms[0].action = 'guiaconversioncajas.do?'+params;
	document.forms[0].banCerrar.value = "false";//bandera para validar cuando se cierre la ventana desde "x" de navegador
	document.forms[0].submit();
}
...
function add_all_li() {
	...
	/*if (document.forms[0].currentTask.value == 'validRefrClnt') {
		scrollWindow();
	}*/
}
...
function validaValoresMaximosSEG(tipo) {
	...
	if (value > valueMAX) {
			alert("El valor de lo largo sobrepasa el valor máximo que es: " + valueMAX);
	...
	if (document.forms[0].volL.value == "" || value == 0) {
		...
		if((document.forms[0].shipType.value != 'ST' || document.forms[0].forceCaptureDimensions.value == 'Y') && (document.forms[0].modificar.value == "Modificar Documentacion")) {//VALIDACION DE SOBRES
			alert(cadenaVolL);
			document.forms[0].volL.focus();
			return false;
		} else {
	...
	if (value > valueMAX) {
			alert("El valor del alto sobrepasa el valor máximo que es: " + valueMAX);
	...
	if (value > valueMAX) { 
			alert("El valor del ancho sobrepasa el valor máximo que es: " + valueMAX);
	...
	if(document.forms[0].shipType.value == 'ST') {
		...
		value = new Number(document.forms[0].volume.value);
		valueMAX = new Number(tipo == 1 ? document.forms[0].volMAX.value : document.forms[0].volMAX.value);
		if (value > valueMAX) {
			alert("El valor del volumen sobrepasa el valor máximo que es:  "+valueMAX + " metros cúbicos.");
			if((vacioVolL == 'Y' && vacioVolH == 'Y' && vacioVolW == 'Y')) {
				document.forms[0].volume.focus();
			} else {
				document.forms[0].volL.focus();
	...
	valueMAX = new Number(document.forms[0].pesoMAX.value);
	if (value > valueMAX) {
		valueWeightOriginal = new Number(document.forms[0].weightOriginal.value);
		if(valueWeightOriginal >= value && document.forms[0].shipType.value != 'ST') {
			alert("El peso real máximo permitido es de " + valueMAX +" kg., la guía cubre en peso volumetrico hasta " + valueWeightOriginal +" kg.");
		} else {
			alert("El valor del peso sobrepasa el valor máximo que es: " + valueMAX);
		}
		document.forms[0].weight.focus();
		return;
	}
