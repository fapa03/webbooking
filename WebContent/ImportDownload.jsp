...
<%
int count=0;
ArrayList listofNames=(ArrayList)session.getAttribute("fileNames");
//AAPP//System.out.println(listofNames.size()+"Sizeingg");
...
{
	//AAPP//System.out.println("inside else");
	%>
	<script>
var count=0;
	</script>
...
document.forms[0].action = "ImportDownload.jsp?count="+count;
...
function downloadPrintGuia(temp){
		isDownloaded=true;

		document.forms[0].action = "DownloadImportFileGuia.jsp?file="+temp;
		document.forms[0].submit();
}
...
<table border="0" cellspacing="0" cellpadding="0" width="769">
  ...
  <table width="769" border="0" cellspacing="0" cellpadding="0">
	...
	<tr> 
	<td  align="center"><b>GUIA NUMBER GENERATED</b></td>
	...
	for(;count<listofNames.size();count++)
	{
		//AAPP//System.out.println(listofNames.get(count));
		table=(java.util.Hashtable)listofNames.get(count);
		java.util.Set keys=table.keySet();
		java.util.Iterator iterate=keys.iterator();
	...
	%>
	<script>
	var te='<%=listofNames.get(count)%>'
	</script>
	
    <tr> 
	 <td  align="center"><%=key%></td>
      <td width="400" align="left" colspan=2>
	  <a href="javascript:downloadPrintGuia('<%=table.get(key)%>')"><img src="images/download_GN.gif"  border="0" width="40%"> </a>
      </td>     
    </tr>
	<%
		if(counter==5)
	 ...
	 <tr> 
      ...
      <td colspan="3"> 
		...
		<%
			...
			{
			%>
			<input type="button" value="Prev" onclick="return prev()" disabled="true">
			<%
			}
			else
			{
			%>
			<input type="button" value="Prev" onclick="return prev()">
			<%
			}
			%>
			...
			<%
			if(count==(listofNames.size()))
			{
			%>
			<input type="button" value="Next" onclick="return next()" disabled="true">
			<%
			}
			else
			{
			%>
			<input type="button" value="Next" onclick="return next()" >
			<%
			}
			%>
